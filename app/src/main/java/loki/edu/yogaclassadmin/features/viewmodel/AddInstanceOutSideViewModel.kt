...
class AddInstanceOutSideViewModel: ViewModel() {
    ...
    val listClasses = mutableStateOf<List<YogaClass>>(emptyList())
    ...
    fun saveInstance(
     ...
     fun loadClasses() {
        viewModelScope.launch {
            yogaClassRepository.getClasses(
                onSuccess = { classes ->
                    listClasses.value = classes
                },
                onFailure = { exception ->
                    Log.e("Error", "Failed to load classes: ${exception.message}")
                }
            )
        }
    }
